...
class _CommunityPageState extends State<CommunityPage> {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        onTap: () => Navigator.pushNamed(context, '/profile/$uid'),
                        child: Column(
                ...
                builder: (context, snap) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(bodyText, maxLines: 3, overflow: TextOverflow.ellipsis),
                      trailing: IconButton(
                        icon: const Icon(Icons.emoji_emotions_outlined),
                        onPressed: () {
                          // quick reaction placeholder — could open reaction sheet
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reagir ao aviso')));
                        },
                      ),
                    ),
                  );
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasError) return const Center(child: Text('Erro ao carregar feed'));
                  ...
                  if (docs.isEmpty) return const Center(child: Text('Nenhuma publicação encontrada.'));
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  CircleAvatar(
                                      radius: 20,
                                      backgroundImage: avatar != null ? CachedNetworkImageProvider(avatar) : null,
                                      child: avatar == null ? Text(author.isNotEmpty ? author[0] : '?') : null),
                                  const SizedBox(width: 8),
                                  Expanded(
                                      child: Text(author,
                                          style:
                                              const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A)))),
                                  if (ts != null)
                                    Text(_formatTimestamp(ts), style: Theme.of(context).textTheme.bodySmall),
                                ],
                              ),
                              if (text.isNotEmpty) ...[
                                const SizedBox(height: 8),
                                Text(text, style: const TextStyle(color: Color(0xFF1A1A1A))),
                              ],
                              if (image != null) ...[
                                const SizedBox(height: 8),
                                ClipRRect(
                              ...
                              Row(
                                ...
                                children: [
                                  IconButton(onPressed: () {}, icon: const Icon(Icons.thumb_up_off_alt)),
  ...
  Widget _categoryChip(String label) {
    final selected = (label == 'Todos' && (_selectedTag == null || _selectedTag == 'Todos')) || (_selectedTag == label);
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 8),
      child: ChoiceChip(
        label: Text(label),
        selected: selected,
        onSelected: (_) => setState(() => _selectedTag = label == 'Todos' ? null : label),
      ),
    );
  }
            ...
            children: [
              ...
              Text(name,
                  ...
                  style: theme.textTheme.bodyMedium
                      ?.copyWith(fontWeight: FontWeight.bold, color: theme.textTheme.bodyMedium?.color)),
              const SizedBox(height: 6),
              Text(bio,
                  ...
                  Row(
                    children: [
                      Container(
                          width: 10,
                          height: 10,
                          decoration: BoxDecoration(color: _presenceColor(context, presence), shape: BoxShape.circle)),
                      const SizedBox(width: 8),
