...
class StorageUploadService {
  ...
  Future<String> uploadBytes(String path, List<int> bytes, {String contentType = 'image/jpeg'}) async {
  ...
  Future<String> uploadFile(String path, File file, {String contentType = 'image/jpeg'}) async {
    final ref = _storage.ref().child(path);
    final meta = SettableMetadata(contentType: contentType);
  ...
  String postImagePath(String postId, {String filename = 'image.jpg'}) => 'posts/$postId/$filename';

  /// Build a path for chat images.
  String chatImagePath(String chatId, int timestamp, {String ext = 'jpg'}) => 'chats/$chatId/$timestamp.$ext';
}
